...
**焦點模式**：用書卷／章／節下拉選單把這次追求的段落（單節或節範圍）一段段加入清單，段數不限，
再一鍵建圖──只畫這些段落彼此之間的串珠與被兩處以上共同引用的橋接經節，不會無限擴張。
焦點清單會寫進網址 hash（`#f=60.2.4-5,38.3.9,…`，即 書卷號.章.起節-迄節），可以直接分享給聖徒；
舊版文字格式的 `#f=` 連結（如 `#f=彼前二4～5，亞三9`）仍可開啟。
