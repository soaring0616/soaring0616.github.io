...
載入時（`src/lib/sundayPicks.ts`）做三件事：每首詩掛上 `usage`（唱過幾次、各段幾次）；把唱過的段落名依次數插到 `categories` **最前面**，
所以擘餅推薦的分段會照你們實際的用法（178 唱過 13 次記念主，就一定在記念主段）；每唱一次熟悉度 +0.05（上限 0.95，只升不降）。
主日在某段唱過的詩也**不受擘餅的 exclude 限制**（調靈段常點福音、渴慕類的詩，例如補充本 836、410）。
卡片上會顯示「主日唱過 13 次（記念主 13）」，頁首的「主日豫選紀錄」可以整份翻閱。
