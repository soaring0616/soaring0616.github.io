...
| `sections` | 分段，例如擘餅的「調靈 → 讚美主 → 記念主 → 敬拜父」。每段有自己的 `include`，但沿用聚會層級的 `exclude` 與 `weights`。**同一首詩只會出現在一個段落**：`categories` 的第一個類別命中哪段就歸哪段（主日唱過的段落、手填的主要類別都在最前面）；第一個沒命中時歸命中最多的段，平手看命中類別誰在 `categories` 裡排得前面。`include` 為空陣列的段落是「一般」段：收下沒命中任何主題段的詩，再用聚會層級的 `include` 過濾（小排、特別聚會是「願意受成全 → 加強 → 一般」；訓練／特會再多切「羨慕主 → 奉獻 → 事奉與召會生活」；福音聚會分「傳揚福音」與「去傳福音」） |
