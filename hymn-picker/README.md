...
卡片上顯示成 `# 憂慮` 這種黃色 chip 和「主題出處」連結。首頁把所有 tags 列成可勾選的 chip（附首數），
勾一個以上就只列帶這些主題的詩歌，命中越多排越前；另外還有一個「自由關鍵字」輸入框，比對標題、首句、類別和 tags，只加分不過濾。`hymns` 可以先列還沒加進 hymns.json 的 id，加進來就自動生效。
