...
chip 依 `groups` 分成七列：心情處境（憂慮、孤單、試煉…）、場合（出遊、晨興、開學、聖誕…）、聖經人物與書卷（亞伯拉罕、雅歌、詩篇二十三篇…）、
愛主與交通（愛主、呼求主名、喫喝主…）、奉獻與跟隨（奉獻、跟隨主、爭戰…）、召會與建造（建造、錫安、國度…）、
帶詩提醒（容易唱錯、難唱、新調、外語…）；沒歸組的放「其他」。同一個 tag 只放一組。`hymns` 可以先列還沒加進 hymns.json 的 id，加進來就自動生效。
...
`hymns.json` 目前的 296 首：大本、補充本、新歌共 270 首，標題、第一句、調號、拍號、韻律、有無副歌、英文號與 `notes` 裡的
